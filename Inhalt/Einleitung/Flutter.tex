...
Die Formular-Anwendung verwendet ausschließlich \IC{StatelessWidget}s, da die Verwaltung des Zustands über das sogenannte BloC Pattern umgesetzt wird.
...
Das \enquote{hot reload} ermöglicht es, während eine Anwendung im Debug-Modus ausgeführt wird, Änderungen an dessen deren Quellcode vorzunehmen.
...
Die Werte werden erneut mit ,(blau unterlegen) getrennt voneinander angegeben (\IC{[1,2]}).
...
Listing \ref{lst:CollectionForInEinerMenge} geht beispielsweise durch die Liste von den Temperatur-Angaben 97.7,105.8, die in Fahrenheit gelistet sind.
Für jeden Schleifendurchlauf wird die Schleifen-Variable f(blau) mit der entsprechenden Formel in Grad Celsius umgewandelt.
