...
Dann sind der \IC{ChangeNotifier} in dem Widget, welches dem Parameter \IC{child} übergeben wird und darüber hinaus allen Kinder-Elementen dieses Widgets verfügbar.?
...
Das Äquivalent zum \IC{Consumer} ist das mitgelieferte Widget \IC{StreamBuilder},
...
Doch das Anbieten des Services durch ein eigens implementiertes \IC{InheritedWidget} erlaubt einen Zugriff,
...
Ein \IC{Sink} implementiert jedoch ausschließlich Methoden zum Hinzufügen von Ereignissen. Um den Stream zu lesen,
...
Es oder er könnte genauso gut durch einen weiteren \IC{StreamController} ersetzt werden.
...
so wird auch die Methode \IC{map} ausgelöst und ein transformiertes Eigenschaft oder Ergebnis? gesendet \Z{6}.
