...
Der Fall, dass die Kernkomponenten an Relevanz verlieren und empfohlen wird, auf externe Bibliotheken zuzugreifen, ist zwar nicht ausgeschlossen, geschieht aber im Wesentlichen \pdfcomment[icon=Note,color=blue]{Einschub ab nächstem Satz}seltener.
...
\begin{alexfigure}{Inhalt/Hauptteil/Technologie-Auswahl/Vergleich.png}
 ...
 auf den Seiten \pageref{lst:VergleichFlutterValidation} und \pageref{lst:VergleichFlutterMain} zu \pdfcomment[icon=Note,color=blue]{27.08.2021 Einschub (nächster Satz))}finden.
 ...
 \pdfcomment[icon=Note,color=blue]{27.08.2021 Einschub bis hier}
...
\begin{itemize}[topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
   ...
   \item \enquote{Flutter}-\enquote{Widgets} bringen ohne zusätzliche Konfiguration bereits ein ansprechendes Aussehen mit. \enquote{React Native} Anwendungen müssen dafür mit zusätzlichen Stylesheets ausgestattet werden.\pdfcomment[icon=Note,color=blue]{27.08.2021 Einschub (nächster Satz))} 
\end{itemize}
...
Kommentare, leere Zeilen und \enquote{import}-Anweisungen wurden bei der Untersuchung ignoriert.\pdfcomment[icon=Note,color=blue]{27.08.2021 "\enquote{import}-Anweisungen" hinzugefügt}
...
Die allgemeine Anzahl der Quelltext-Zeilen ist für die Evaluierung der Technologie deshalb zweitrangig. \pdfcomment[icon=Note,color=blue]{27.08.2021 Dieser Satz ist neu}
...
die insgesamt für die Entwicklung des Formulars in \enquote{Flutter} nötig waren. \pdfcomment[icon=Note,color=blue]{27.08.2021 Wort "Zeilenanzahl" eingefügt}

 


\subsection{Automatisiertes Testen}\pdfcomment[icon=Note,color=blue]{Einschub von hier}
...
Weiterhin wird auf die \enquote{Jest}-API-Dokumentation verwiesen, sowie auf ein Beispiel für einen \enquote{Snapshot}-Test in der \enquote{Jest}-Dokumentation.\footnoteL{\url{https://jestjs.io/docs/snapshot-testing}}\pdfcomment[icon=Note,color=blue]{Snapshot-Test kursiv gemacht}
...
\pdfcomment[icon=Note,color=blue]{<- Dieser Satz wurde aktualisiert} Herausgegeben wird die \enquote{React Native Testing Library} vom Drittanbieter \enquote{Callstack} -- einem Partner im \enquote{React Native}-Ökosystem.\footcite[Vgl.][]{TheReactNativeEcosystem}
...
wie ein solcher Test erstellt wird. \pdfcomment[icon=Note,color=blue]{\enquote{eine bis mehrere} ersetzt durch \enquote{auf eine oder mehrere}}
