...
die einen Wahrheitswert \IC{bool} zurückgibt und als Parameter die Menge aller bisher ausgewählten Auswahloptionen vom Typ \IC{Set<Choice>} übergeben bekommt. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
In diesem Fall sind das alle \enquote{Subjekte} des \enquote{ViewModels} \Z{25-29}.\pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
Somit tauchen keine Null-Werte in der Menge auf und damit kann der \enquote{Stream} mit dem Typargument \IC{Set<Choice>} ohne Null-Zulässigkeit deklariert werden \Z{24}. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
Diese Kopie hätte den Zustand einer Momentaufnahme aller bisherigen Auswahloptionen zum Zeitpunkt der Konstruktion der Selektionskarte.\pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
Alle Änderungen,
...
so wird die \IC{validator}-Funktion jeweils \IC{null} statt einer Fehlermeldung zurückgeben \Z{138}. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
