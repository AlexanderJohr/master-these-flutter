...
Damit der Hashtabelle der Schlüssel \IC{'massnahmenCharakteristika'} hinzugefügt wird, muss der entsprechende Eintrag im Wertetyp \IC{Massnahme} hinzugefügt werden.
Die \enquote{Getter}-Methode \IC{massnahmenCharakteristika}, die das Paket \enquote{built_value} dazu veranlasst,
...
Ist der dagegen nicht \IC{null}, so wird der Wert links vom Operator \IC{??} genutzt.
\DartSpec{165} Ist der Wert also nicht gefüllt, so wird in allen Fällen der leere \IC{String} \IC{""} als Argument übergeben.
...
Damit die Eingabefelder die neuen Werte eintragen können, muss das \enquote{ViewModel} die beobachtbaren \enquote{Subjects} bereitstellen \LstZ{\ref{lst:Schritt2MassnahmencharakteristikaWerdenDemViewModelHinzugefuegt}}{12-17}.
...
um das Objekt des geschachtelten Wertetyps \IC{Identifikatoren} anzupassen \LstZ{\ref{lst:Schritt2KonvertierungDesViewModelsInDasModel}}{44}.
So ist es auch für den geschachtelten Wertetyp \IC{MassnahmenCharakteristika} der Fall.
...
Die Abkürzung \IC{LTRB} steht dabei für left, top, right, bottom -- deutsch links, oben, rechts, unten.
