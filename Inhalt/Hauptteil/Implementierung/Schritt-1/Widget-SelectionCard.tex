...
Das Listing \ref{lst:Schritt1KlasseSelectionCard} zeigt die Struktur des Widgets SelectionCard.
...
Zu diesem Zweck wandelt die Methode \IC{map} alle Elemente von \IC{selectedChoices} in \IC{String}-Objekte um, indem es von dem \IC{Choice}-Objekt lediglicht den Beschreibungstext \IC{description} verwendet.
...
Damit ist er außerhalb des \IC{StreamBuilder} und bleibt somit beim Neuzeichnen der Karte erhalten.
...
Da das Wechseln zu Unterseite bevorsteht, fordert der \IC{focusNode} den Fokus für das angeklickte \IC{ListTile} an \Z{38}.
...
Die \IC{replace}-Methode des Builder-Objektes kann die gesamte Kollektion im \IC{BuiltSet} austauschen, ungeachtet dessen, dass es sich beim Argument selbst nicht um ein \IC{BuiltSet} handelt.
