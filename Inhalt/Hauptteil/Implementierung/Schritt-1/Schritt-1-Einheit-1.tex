\chapter{Schritt 1 -- Grundstruktur des Formulars}
...
Das Schlüsselwort \mintinline{dart}{enum} in \enquote{Dart} erlaubt lediglich die Auflistung konstanter Symbole\footcite[Vgl.][S. 74f.]{DartProgrammingLanguageSpecification5thedition}.
...
\IC{this.description = description}.\DartSpec{40f} \pdfcomment[icon=Note,color=yellow]{28.08.2021 2. Der ganze Absatz wurde neu formuiert}
...
Damit ist garantiert, dass die Instanzvariable \IC{name} einen Wert erhält,  der nicht \enquote{null} ist.  \pdfcomment[icon=Note,color=yellow]{28.08.2021 Ganzer Absatz umformuliert}

Neben \IC{name} wird mit \IC{choiceByAbbreviation} eine weitere Instanzvariable deklariert \Z{12}.
Es handelt sich um den Datentyp \IC{Map} -- eine Kollektion, welche Daten mittels Schlüssel-Werte-Paaren ablegen kann. \pdfcomment[icon=Note,color=yellow]{28.08.2021 , welche}
...
Auffällig dabei ist, dass der Parameter \IC{abbreviation} mit dem Typ \IC{String?} und der generische Rückgabetyp mit \IC{T?} gekennzeichnet ist. Der Suffix \IC{?} macht beide zu Typen mit Null-Zulässigkeit.
...
sondern existieren bereits als Plugin für die beiden Entwicklungsumgebungen\footnoteL{https://plugins.jetbrains.com/plugin/13786-built-value-snippets}\textsuperscript{,}\footnoteL{https://marketplace.visualstudio.com/items?itemName=GiancarloCode.built-value-snippets}.
...
Dabei handelt es sich schlicht um einen Funktions-Körper, der aus genau einer Anweisung besteht. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
Beispiel: Die zwei Aufrufe \IC{objekt.tueEtwas();} und \IC{objekt.tueEtwasAnderes();} sind äquivalent mit dem Einzeiler \IC{objekt..tueEtwas()..tueEtwasAnderes();}. \pdfcomment[icon=Note,color=yellow]{28.08.2021 Satz umformuliert}
...
welches im Eingabeformular durch das Texteingabefeld gefüllt wird.\pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
Der initiale Wert wird auf einen konstanten Wert gesetzt, der dem Zustand \IC{'in Bearbeitung'} entspricht \Z{54}. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
sondern wird einmalig auf den aktuellen Zeitstempel gesetzt \Z{53}. \pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
...
Die Spalte \enquote{Zuletzt bearbeitet am} enthält die Datumsangaben \enquote{2021-8-4 13:20} und \enquote{2021-8-4 13:17}. \pdfcomment[icon=Note,color=yellow]{28.08.2021 Satz hinzugefügt}
...
der in Zeile 9 übergeben wurde.\pdfcomment[icon=Note,color=yellow]{29.08.2021 Satz umformuliert}
