...
Der \IC{Future}, der von der Funktion zurückgegeben werden soll, muss in der Zukunft den Wert \enquote{true} zurückgeben, wenn dem \enquote{Navigator} erlaubt  werden soll, zurück zu navigieren.
...
Der \enquote{Stream} ist mit dem generischen Typ \IC{BuiltSet<ChoiceType>} konfiguriert \Z{17}.
...
Wenn nun der Benutzer der \IC{SelectionCard} einen Typ wie etwa \enquote{LetzterStatus} für den Typparameter übergibt,
so erhält er auch die Rückruffunktionen \IC{onSelect} und \IC{onDeselect},
deren erster Parameter vom Typ \enquote{LetzterStatus} ist.
Ohne eine Typumwandlung
-- englisch \enquote{type casting} --
von \enquote{Choice} in \enquote{LetzterStatus} können so Operationen auf dem Objekt angewendet werden, die nur die Klasse \enquote{LetzterStatus} unterstützt.

Das erste \enquote{Widget}, welches von der \IC{build}-Methode zurückgegeben wird, ist ein \IC{StreamBuilder} \LstZ{\ref{lst:Schritt1BuildMethodeDerSelectionCard}}{47}.
...
Ein \IC{ListTile}-\enquote{Widget} erlaubt es dann, den übergebenen \IC{title} als Überschrift zu setzen \Z{54} und die aktuell ausgewählten Selektionen als Untertitel anzuzeigen \Z{56}.
Zu diesem Zweck wandelt die Methode \IC{map} alle Elemente von \IC{selectedChoices} in \enquote{String}-Objekte um,
indem es von dem \enquote{Choice}-Objekt \IC{c} lediglich den Beschreibungstext \IC{description} verwendet.
Anschließend sammelt der Befehl \IC{join} die resultierenden \enquote{String}-Objekte ein,
formt sie in einem gemeinsamen \enquote{String} zusammen und trennt sie darin jeweils mit einem \IC{","} voneinander.
...
Sie wird dem Parameter \IC{onTap} des \IC{ListTile}-\enquote{Widgets} übergeben \Z{58}.
Da das Wechseln zur Unterseite bevorsteht, fordert der \IC{focusNode} den Fokus für das angeklickte \IC{ListTile} an \Z{38}.
Schließlich navigiert der Benutzer mit \IC{Navigator.push} zur Unterseite \Z{40}.
...
Hier wird der Vorteil der dynamischen Erzeugung der Seite offensichtlich: Die Unterseite kann das gleiche \enquote{ViewModel} wiederverwenden,
welches auch von der \enquote{SelectionCard} genutzt wird.
Auch alle weiteren Instanzvariablen der \enquote{SelectionCard} können wiederverwendet werden.
...
Dadurch, dass die \enquote{SelectionCard} und der Selektionsbildschirm sich das gleiche \enquote{ViewModel} teilen,
...
Die Optionen werden in einem \enquote{ListView} präsentiert \Z{73}.
Er ermöglicht es, Listen-Elemente in einem vertikalen Scrollbereich darzustellen.
Die Funktion \IC{map} konvertiert alle Objekte in der Liste aller möglichen Optionen \IC{allChoices} \IC{74} in Elemente des Typs \IC{CheckboxListTile} \Z{77}.
...
Das \IC{CheckboxListTile} erhält einen Titel, der aus dem Beschreibungstext \IC{description} des \enquote{Choice}-Objektes \IC{c} gebildet wird \Z{81}.
...
Eine Fallunterscheidung überprüft zunächst, ob der Parameter \IC{selected} nicht \IC{null} ist \Z{84},
denn sein Parametertyp \IC{bool?} lässt Null-Werte zu \Z{83}.
Durch die Typ-Beförderung ist \IC{selected} innerhalb des Körpers der Fallunterscheidung dann vom Typ \enquote{bool} ohne Null-Zulässigkeit \Z{85-93}. 


Darin wird zunächst der Zustand des \enquote{ViewModels} der \enquote{SelectionCard} aktualisiert \Z{85-88}.
Die \IC{replace}-Methode des \enquote{Builder}-Objektes kann die gesamte Kollektion im \enquote{BuiltSet} austauschen \Z{87},
ungeachtet dessen, dass es sich beim Argument selbst nicht um ein \enquote{BuiltSet} handelt.
Die \IC{replace}-Methode wandelt das Argument dafür automatisch um.
Wenn die Auswahloption ausgewählt war, wird sie beim Anklicken abgewählt, indem eine leere Liste \IC{[]} als neuer Wert gesetzt wird.
Wurde die Auswahloption aber angewählt, so wird eine Liste mit der Option als einziges Element übergeben: \IC{[c]}. \pdfcomment[icon=Note,color=yellow]{30.08.2021 Die letzten zwei Sätze sind neu}
Durch Zuweisung des neuen Wertes erhält das \enquote{ViewModel} der \enquote{SelectionCard} ein neues Ereignis.
Damit werden die \enquote{SelectionCard} und der dazugehörige Selektionsbildschirm aktualisiert.
Während der Erstellung dieser Arbeit wurde versucht, die \enquote{SelectionCard} als ein \enquote{StatefulWidget} zu erstellen.
Mittels \enquote{setState} sollte dafür gesorgt werden, dass sowohl \enquote{SelectionCard} als auch der Selektionsbildschirm aktualisiert werden.
Doch bei diesem Vorgehen zeichnet sich nur die \enquote{SelectionCard} neu.
Der Selektionsbildschirm bleibt unverändert, denn er wird zwar von der \enquote{SelectionCard} gebaut, doch ist er nicht tatsächlich Kind der \enquote{SelectionCard}.
In Wahrheit ist der Selektionsbildschirm ein Kind von \enquote{MaterialApp} -- genau wie \enquote{MassnahmenMasterScreen} und \enquote{MassnahmenDetailScreen}.

Neben dem \enquote{ViewModel} der \enquote{SelectionCard} muss jedoch auch das \enquote{ViewModel} der Eingabemaske aktualisiert werden.
