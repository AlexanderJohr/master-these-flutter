...
Flutter erlaubt über einen eigenen Testtreiber solche Integrationstest durchzuführen.
...
Anschließend kann mit der Annotation and generate mocks \LstZ{\ref{lst:Schritt1IntegrationsTestInitialisierung}}{20} ein Mock für \IC{MassnahmenJsonFile} angefordert werden.
...
Um Objekte während des Testens in Oberfläche zu finden, stellt die Klasse \IC{Finder} nützliche Funktionalitäten zur Verfügung.
...
Angenommen der Test ruft \IC{tabSelectionCard} mit dem Argument \IC{letzterStatusChoices} auf, so entspricht choices.name dem String \IC{"Status"}. Der Ausdruck \IC{find.text("Status")} lokalisiert den Titel innerhalb der Selektions-Karte \Z{50}. 
...
Wurde das Text-Element gefunden, so muss noch den Vater gesucht werden, der vom Typ \IC{Card} ist \Z{53}.
Das kann mit \IC{find.ancestor} erfolgen. Über den Parameter \IC{of} erhält er den \IC{Finder} des Kind-Elements und der Parameter \IC{matching} erhält als Argument die Voraussetzung, die vom Vater-Objekt erfüllt werden soll, als weiteren \IC{Finder}. \IC{find.byType(Card)} sucht also alle Elemente vom Typ \IC{Card}. \IC{find.ancestor} sucht anschließend alle Entsprechungen, in der eine \IC{Card} ein Vater des \IC{Finder textLabel } ist. Wiederum überprüft die Funktion \IC{expect}, dass die Karte gefunden wurde. Doch dieses Mal muss es genau ein Widget sein, welches mit dem \enquote{Matcher} \IC{findsOneWidget} verifiziert werden kann \Z{54}. Sollte mehr als nur eine Karte gefunden werden, so wäre nicht klar, welche geklickt werden soll.
...
Die Hilfsfunktion klickt den Button \Z{63} und wartet dann erneut auf Vollendung aller angestoßenen Animationen \Z{64}.
...
Ist der Integrationstest aktuell in dem Auswahlbildschirm, so sorgt \IC{tabOption} dafür, dass Auswahloptionen gewählt wird \Lst{\ref{lst:Schritt1HilfsmethodeTabOption}}.
...
Ein Klick auf das Text-Label reicht bereits aus, denn damit wird das Vater-Element - das \IC{CheckboxListTile} - ebenfalls getroffen.
Der \IC{tester} holt es in den sichtbaren Bereich \IC{71}, klickt es \IC{72} und wartet auf Abschluss aller Animationen \Z{73}.
...
Sie findet das \IC{TextFormField}, indem es zunächst nach dem Titel mit \IC{find.text(title)} und anschließend dessen Vater-Element vom Typ \IC{TextFormField} sucht \Z{83}. Sollte sowohl der Hinweistext als auch der Titel den gleichen Text enthalten, so kann es sein, dass zwei solche Elemente gefunden werden. In Wahrheit ist es aber zwei Mal dasselbe \IC{TextFormField}. Mit \IC{.first} wird lediglich das erste Element geliefert \Z{85}.
...
Während der Integrationstest startet, öffnet sich als Erstes der Übersichts-Bildschirm.
...
Es folgt der Klick auf den Button zum Erstellen einen neuen Maßnahme \Z{95}.
...
Das Model ruft aber dennoch die entsprechenden Methoden - wie zum Beispiel \IC{saveMassnahmen} - auf.
...
Die lokalen Variable \IC{actualMassnahme} speichert sie zwischen \Z{113}.
