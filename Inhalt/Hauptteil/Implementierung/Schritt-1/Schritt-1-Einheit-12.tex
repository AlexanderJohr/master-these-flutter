...
Damit das Paket \enquote{source_gen} die entsprechenden Dateien analysiert, müssen sie in der Rubrik \IC{sources} angegeben werden \LstZ{\ref{lst:Schritt1IntegrationsTestBuildYaml}}{3-8}.
Wird der Ordner \enquote{integration_test} darin eingefügt \Z{8}, bezieht \enquote{source_gen} den Ordner in die Codegenerierung mit ein.
...
\begin{alexyamllisting}{Schritt 1}{Initialisierung des Integrationstests}
  ...
  {firstline=0}
  \label{lst:Schritt1IntegrationsTestBuildYaml}
\end{alexyamllisting}
...
Anschließend wird ein Objekt der generierten Klasse \IC{MockMassnahmenJsonFile} erstellt \Z{27}.
...
Doch der Konstruktor von \IC{MassnahmenModel} erhält dieses Mal nicht das \enquote{MassnahmenJsonFile}, sondern den entsprechenden \enquote{Mock} \Z{31}. 
...
Über die Fabrikmethode \IC{text} wird ein \enquote{_TextFinder} konstruiert.
...
denn \IC{Finder titel = find.text("Maßnahmentitel")} ist  deutlich leichter zu erfassen als
\IC{Finder titel = new _TextFinder("Maßnahmentitel")}.



Um die Selektionskarten zu finden, wird lediglich der Titel-Text benötigt.
Angenommen, der Test ruft \IC{tabSelectionCard} mit dem Argument \enquote{letzterStatusChoices} auf,
so entspricht \IC{choices.name} dem \enquote{String} \enquote{``Status''}.
Der Ausdruck \IC{find.text(choices.name)} lokalisiert den Titel innerhalb der Selektionskarte \Z{50}.
...
Der Aufruf von \IC{expect} mit dem entsprechenden \IC{Finder}-Objekt und dem \enquote{Matcher} \IC{findsWidgets} verifiziert,
dass mindestens ein entsprechendes \enquote{Text}-Element gefunden wurde.

Wurde das \enquote{Text}-Element gefunden, so muss noch der Vater gesucht werden, der vom Typ \IC{Card} ist \Z{53}.
...
Die Methode \IC{ensureVisible} scrollt den Bildschirm zur entsprechenden Position, damit die Karte sichtbar ist \Z{56}.
...
Sollte der optionale Parameter \IC{tabConfirm} auf \enquote{true} gesetzt sein \Z{75}, so wird der Selektionsbildschirm anschließend direkt wieder geschlossen, nachdem die Option ausgewählt wurde  \Z{76}. 
...
Mit \IC{.first} wird lediglich das erste Element geliefert \Z{84}.
...
Der Integrationstest öffnet nun die Eingabemaske, in der die Selektionskarte zum Setzen des letzten Status angewählt wird \LstZ{\ref{lst:Schritt1LetzterStatusWirdAusgewählt}}{98}.
Anschließend fällt die Wahl auf die Option für \enquote{abgeschlossen} \Z{99}.
...
Es erfolgt die Erstellung eines beispielhaften Titels anhand des aktuellen Datums und der aktuellen Uhrzeit \Z{101-103}.
...
Die Rückgabe ist vom Typ \enquote{VerificationResult} und enthält eine \enquote{Getter}-Methode mit dem Namen \IC{captured}.
...
Dazu reicht es,
die entsprechenden Schlüssel-Werte-Paare über die Schlüssel \IC{"guid"} und
\IC{"letztesBearbeitungsDatum"} aus der Ergebnis-Hashtabelle zu entfernen \Z{114-115}.
