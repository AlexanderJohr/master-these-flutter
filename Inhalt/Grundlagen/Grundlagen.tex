...
Für die Formular Anwendung wurde die Programmiersprache \enquote{Dart} und das Frontend-Framework \enquote{Flutter} gewählt.
...
\enquote{Flutter} ist inspiriert durch das Web-Framework \enquote{React} und deren Oberflächenelemente, die \enquote{Components} genannt werden \footcite[Vgl.][]{IntroductionToWidgets}.
...
\enquote{Flutter} unterscheidet allerdings zwischen zwei Arten von Widgets: denen, die einen Zustand pflegen -- den \enquote{Stateful Widgets} -- und solchen, die keinen Zustand haben -- den \enquote{Stateless Widgets}.
...
Das kann unter anderem der Fall sein, wenn das Widget zum ersten Mal in der Oberfläche auftaucht, oder das Vaterelement und damit alle Kinderelemente neu gezeichnet werden.
\HP{oder es von Inherited Widget abhängt?}

\enquote{StatefulWidgets} sind nur eine von vielen Möglichkeiten den Zustand des Programms zu verwalten.
Die Formularanwendung verwendet ausschließlich \IC{StatelessWidget}s, da die Verwaltung des Zustands über das sogenannte BloC Pattern umgesetzt wird.
Mehr dazu im Kapitel \HP{Kapitel einfügen}.
\HP{BLoC Pattern erklären? Oder einfach bei BehaviourSubject verweisen, dass MVVM bei Google BloC genannt wird (einfacher)}

\HP{Inherited Widgets?}
...
Für die Ziel-Architekturen ARM32, ARM64 und x86_64 wird \enquote{Dart} in Maschinencode kompiliert \footcite[Vgl.][]{DartThePlatforms}.

Dementsprechend kommt während der Entwicklung eine virtuelle Maschine -- die \enquote{Dart VM} -- über Just-in-time-Kompilierung (JIT) zum Einsatz.
...
Das Hauptprogramm importiert über das Schlüsselwort \IC{import} Funktionalitäten aus  weiteren \enquote{Dart}-Dateien oder sogar ganzen Bibliotheken.
...
\enquote{Dart} greift daher auf eine andere Variante der Metaprogrammierung zurück: die Quellcode-Generierung.
...
\subsection{Set und Map Literale}

\enquote{Dart} erlaubt es Listen (\IC{List}), Mengen (\IC{Set}) und Hashtabellen (\IC{Map}) als sogenannte Literale zu deklarieren.
...
\subsubsection{Collection for} \enquote{Dart} erlaubt es Schleifen innerhalb von Listen-, Mengen- und Hashtabellen-Literalen zu verwenden.
...
\end{minted}
    \caption[Das Collection-for in einer Menge]{Das Collection-for in einer Menge, Quelle: Eigenes Listing}
    \label{lst:CollectionForInEinerMenge}
  \end{listing}
\fi

Für jeden Schleifendurchlauf wird die Schleifen-Variable \IC{f} mit der entsprechenden Formel in Grad Celsius umgewandelt.
...
\end{minted}
    \caption[Das Collection-for in einer Hashtabelle]{Das Collection-for in einer Hashtabelle, Quelle: Eigenes Listing}
...
\end{minted}
    \caption[Collection-if in einer Liste]{Das Collection-if in einer Liste, Quelle: Eigenes Listing}
...
\subsection{Typen ohne Null-Zulässigkeit} Im Vergleich zu vielen anderen Programmiersprachen -- wie beispielsweise Java -- wird in \enquote{Dart} zwischen gewöhnlichen Typen und nullable Typen unterschieden.
...
Der Compiler liefert jedoch einen Fehler, da der Wert der Variablen \IC{null} sein kann, wie die Notation \IC{int?} anzeigt.
...
In \enquote{Dart} wird syntaktisch nicht zwischen dem Aufruf einer Getter-Methode oder einer Instanzvariablen unterschieden.
...
Genauso könnte es aber auch sein, dass eine Klasse von \IC{Patient} erbt und das Feld \IC{temperature} mit einer gleichnamigen Getter-Methode überschreibt.
...
\end{minted}
    \caption[Überschreiben des Instanzattributs mit einer Getter-Methode]{Überschreiben des Instanzattributs mit einer Getter-Methode, Quelle: Eigenes Listing}
...
Sollte sich der Entwickler sicher sein, dass die Variable nicht \IC{null} sein kann, so kann er mit einem nachgestellten \IC{!} erzwingen, dass die Variable als nicht \IC{null} angesehen wird \LstZ{\ref{lst:printTemperatureInCelsiusLocalVariableForceNullCheck}}{3}.
...
Sollte es dann dennoch passieren, dass die Variable \IC{null} ist, so wird eine Fehlermeldung beim Aufruf der Variablen geworfen.
...
Er gleicht damit dem sogenannten \IC{Promise} -- deutsch Versprechen -- in JavaScript  \footcite[Vgl.][]{PromiseJavaScriptMDN}.
...
Um genau das zu verhindern, existieren in \enquote{Dart} die Schlüsselwörter \IC{async} und \IC{await}.
...
zeigt, dass das Anwenden des Schlüsselwortes \IC{await} vor der Operation \IC{file.readAsString} dafür sorgt, dass der zukünftige Wert direkt in \IC{fileContent} gespeichert wird.
