...
\HP{oder es von Inherited \enquote{Widget} abhängt?}
...
Im Fall einer \enquote{Map} werden der Schlüssel und der Wert durch einen \IC{:} voneinander getrennt und die Schlüssel-Wertepaare wiederum durch \IC{,} getrennt nacheinander aufgelistet (\IC{{1: "erster Wert",2:"zweiter Wert"}}).
...
Nur in dem Fall, dass die Temperatur der Schleifenvariable \IC{c} größer oder gleich 38,5 ist, wird die Temperatur der Liste zugefügt.
...
dass eine Variable niemals den Wert \IC{null} haben kann. 
Das ist besonders dann nützlich, 
wenn auf einem Objekt eine Methode aufgerufen wird. 
Ist das Objekt in Wahrheit \IC{null}, 
so gibt es erst zur Laufzeit einen Fehler, 
da die Methode auf der Referenz \IC{null} nicht aufgerufen werden kann. 
Damit ein Laufzeitfehler geworfen werden kann, 
muss vor jedem Aufruf einer Methode auf einer Referenz überprüft werden, ob die Referenzen nicht \IC{null} sind. 
...
die niemals den Wert \IC{null} annehmen kann, 
...
sondern zusätzlich auch noch den Wert \IC{null}.
...
Beim ersten Aufruf von \IC{temperature} wird die Zählvariable \IC{counter} von \IC{0} auf \IC{1} erhöht.
...
Er gleicht damit dem sogenannten \IC{Promise} -- deutsch Versprechen -- in \enquote{JavaScript}  \footcite[Vgl.][]{PromiseJavaScriptMDN}.
