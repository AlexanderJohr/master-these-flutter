...
\clearpage
In imperativen Frameworks werden Befehle genutzt, um Änderungen direkt in an Oberfläche vorzunehmen,
...
Die bei jedem Klick wird die \enquote{Setter}-Methode \IC{setText} genutzt, um den Text direkt zu manipulieren.
...
In deklarieren Frontend-Frameworks wie \enquote{Flutter} existieren solche \enquote{Setter}-Methoden nicht.
...
Mehr dazu im Kapitel \ref{sec:ModelViewViewModel}.
...
\subsection{Typen ohne Null-Zulässigkeit} Im Vergleich zu vielen anderen Programmiersprachen -- wie beispielsweise Java -- wird in \enquote{Dart} zwischen Typen mit und ohne Null-Zulässigkeit unterschieden.\pdfcomment[icon=Note,color=yellow]{27.08.2021 Satz angepasst}
In Java ist es nur bei atomaren Datentypen wie \enquote{int} und \enquote{float} verpflichtend einen Wert anzugeben.
